...
import com.com253.payrollsystem.model.HolidayType;
...
public class Menu {
    ...
    /**
     * Runs the CLI payroll menu loop using a caller-supplied scanner.
     * The scanner is owned by the caller and is not closed here.
     *
     * @param sharedScanner scanner to read user input from
     */
    public static void run(Scanner sharedScanner) {
    ...
    static TimeRecord readTimeRecord(int dayNumber) {
        ...
        if (isAbsent) {
            return new TimeRecord(dayNumber, 0, 0, true, HolidayType.NONE);
        }
        ...
        HolidayType holidayType = mapHolidayType(holidayChoice);

        return new TimeRecord(dayNumber, timeIn, timeOut, false, holidayType);
    }

    private static HolidayType mapHolidayType(int holidayChoice) {
        switch (holidayChoice) {
            case 2:
                return HolidayType.REGULAR_HOLIDAY;
            case 3:
                return HolidayType.SPECIAL_OR_REST_DAY;
            default:
    ...
    static PayrollSettings readPayrollSettings() {
        ...
        return new PayrollSettings(
                workingDaysPerMonth,
                TimeConversions.hhmmToHours(workdayStart),
                TimeConversions.hhmmToHours(overtimeStart),
                TimeConversions.hhmmToHours(lunchStart),
                regularLeave,
                probationaryLeave,
                contractualLeave,
                partTimerLeave);
    ...
    static void printPayslip(PayrollEntry entry) {
        ...
        System.out.println("\n");
        System.out.println("============================================================");
        // Company name is hardcoded for now; future work sources it from configuration
        // (see .docs/05-payroll-system-gap-analysis.md).
        ...
        System.out.printf("  Type          : %s%n",  emp.getEmployeeType().getLabel());
